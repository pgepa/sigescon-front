...
function SearchableSelect({
  ...
  canCreateNew = false,
  onCreateNew,
  createNewLabel = "Criar novo usuário",
  maxHeight = "h-80",
  itemNounPlural = "usuários",
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <span
          className={`block truncate ${!selectedOption ? "text-gray-500" : "text-blue-900"}`}
        >
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-blue-200 rounded-lg shadow-xl">
          ...
          <div className="p-3 border-b border-blue-100 bg-blue-50/50">
            ...
            {options.length > 0 && (
              <div className="mt-2 text-xs text-blue-600 flex items-center justify-between">
                <span>
                  {filteredOptions.length} de {options.length} {itemNounPlural}
                </span>
                {options.length >= 100 && itemNounPlural === "usuários" && (
                  <span className="text-amber-600">
                    ⚠ Lista limitada a 100 usuários
                  </span>
                )}
              </div>
            )}
          ...
          <div
            className={`${maxHeight} overflow-y-auto scrollbar-thin scrollbar-thumb-blue-200 scrollbar-track-blue-50`}
          >
...
export function NovoContrato() {
  ...
  const [showCreateContratadoModal, setShowCreateContratadoModal] =
    useState(false);
  ...
  const [termosContratuaisCatalog, setTermosContratuaisCatalog] = useState<
    ...
    async function fetchData() {
      try {
        const [
          ...
          usuariosResponse,
          perfisResponse,
          termosContratuaisResponse,
        ] = await Promise.all([
          fetch(
            `${import.meta.env.VITE_API_URL}/contratados?page=1&per_page=100`,
            {
              method: "GET",
              headers: {
          ...
          fetch(
            `${import.meta.env.VITE_API_URL}/usuarios?page=1&per_page=100`,
            {
              method: "GET",
              headers: {
                Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        ...
        const contratadosArray = Array.isArray(contratadosData)
          ? contratadosData
          : contratadosData.data || [];
        const modalidadesArray = Array.isArray(modalidadesData)
          ? modalidadesData
          : modalidadesData.data || [];
        const statusArray = Array.isArray(statusData)
          ? statusData
          : statusData.data || [];
        const perfisArray = Array.isArray(perfisData)
          ? perfisData
          : perfisData.data || [];
        const termosArrayRaw = Array.isArray(termosContratuaisData)
          ? termosContratuaisData
          : termosContratuaisData?.data || [];

        setContratados(
          contratadosArray.filter((item: any) => item.ativo !== false),
        );
        setModalidades(
          modalidadesArray.filter((item: any) => item.ativo !== false),
        );
        setStatusList(statusArray.filter((item: any) => item.ativo !== false));
        setPerfis(perfisArray);
        setTermosContratuaisCatalog(
          termosArrayRaw.map((t: { id: number; nome: string }) => ({
            id: t.id,
            nome: t.nome,
          })),
        );
        ...
        const [usuariosGestoresResponse, usuariosFiscaisResponse] =
          await Promise.all([
            fetch(
              `${import.meta.env.VITE_API_URL}/usuarios?perfil=Gestor&per_page=100`,
              {
                method: "GET",
                headers: {
                  Authorization: `Bearer ${localStorage.getItem("authToken")}`,
            ...
            fetch(
              `${import.meta.env.VITE_API_URL}/usuarios?perfil=Fiscal&per_page=100`,
              {
                method: "GET",
                headers: {
                  Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        ...
        const gestoresArray = Array.isArray(usuariosGestoresData)
          ? usuariosGestoresData
          : usuariosGestoresData.data || [];
        const fiscaisArray = Array.isArray(usuariosFiscaisData)
          ? usuariosFiscaisData
          : usuariosFiscaisData.data || [];

        setUsuariosGestores(
          gestoresArray.filter((item: any) => item.ativo !== false),
        );
        setUsuariosFiscais(
          fiscaisArray.filter((item: any) => item.ativo !== false),
        );
      } catch (err) {
        console.error("Erro ao carregar opções:", err);
        toast.error(
          "Erro ao carregar dados iniciais: " + (err as Error).message,
        );
      }
    }
    fetchData();
  ...
  const handleSubmitNewUser = async () => {
    ...
    try {
      ...
      if (!createUserResponse.ok) {
        const errorData = await createUserResponse
          .json()
          .catch(() => ({ message: "Erro desconhecido" }));
        throw new Error(
          errorData.message ||
            errorData.detail ||
            `Erro ${createUserResponse.status}: ${createUserResponse.statusText}`,
        );
      }
      ...
      if (selectedPerfis.length > 0) {
        ...
        const grantResponse = await fetch(
          `${import.meta.env.VITE_API_URL}/usuarios/${newUser.id}/perfis/conceder`,
          {
            method: "POST",
            headers,
            body: JSON.stringify({
        ...
        if (!grantResponse.ok) {
          const errorData = await grantResponse
            .json()
            .catch(() => ({ message: "Erro desconhecido" }));
          console.warn(
            "Erro ao conceder perfis:",
            grantResponse.status,
            errorData,
          );
          toast.error(
            "Usuário criado, mas houve erro ao conceder alguns perfis",
            { id: toastId },
          );
        } else {
      ...
      const perfisNomes = selectedPerfis
        .map((perfilId) => {
  ...
  const handleSubmitNewContratado = async () => {
    ...
    try {
      ...
      // Criar contratado
      const createContratadoResponse = await fetch(
        `${import.meta.env.VITE_API_URL}/contratados`,
        {
          method: "POST",
          headers,
      ...
      if (!createContratadoResponse.ok) {
        const errorData = await createContratadoResponse
          .json()
          .catch(() => ({ message: "Erro desconhecido" }));
        throw new Error(
          errorData.message ||
            errorData.detail ||
            `Erro ${createContratadoResponse.status}: ${createContratadoResponse.statusText}`,
        );
      }
  ...
  async function onSubmit(data: ContractFormData) {
    ...
    try {
      ...
      if (totalSize > MAX_TOTAL_SIZE) {
        throw new Error(
          `Tamanho total dos arquivos excede o limite de 50MB (atual: ${(totalSize / 1024 / 1024).toFixed(2)} MB)`,
        );
      }
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/contratos`,
        {
          method: "POST",
          body: formData,
          headers: {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          throw new Error(
            "Arquivo muito grande. Reduza o tamanho do arquivo e tente novamente.",
          );
        } else if (response.status === 422) {
          const errorData = await response
            .json()
            .catch(() => ({ message: "Dados inválidos" }));
          throw new Error(
            `Dados inválidos: ${errorData.message || errorData.detail || "Verifique os campos obrigatórios"}`,
          );
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      if (newTotalSize > MAX_TOTAL_SIZE) {
        ...
        toast.error(
          `Tamanho total excede o limite de 50MB (atual: ${totalMB} MB). Remova alguns arquivos.`,
        );
        event.target.value = "";
        return;
      }
      ...
      if (largeFiles.length > 0) {
        toast.error(
          `Arquivos muito grandes (máx. 10MB cada): ${largeFiles.join(", ")}`,
        );
      }
      ...
      if (validFiles.length > 0) {
        ...
        toast.success(
          `${validFiles.length} arquivo(s) adicionado(s) com sucesso!`,
        );
        console.log(
          "Arquivos válidos adicionados:",
    ...
    <div className="w-full mx-auto p-6">
      ...
      >
        ...
        <div className="col-span-1">
          ...
          />
          {errors.nr_contrato && (
            <p className="text-red-500 text-sm">{errors.nr_contrato.message}</p>
          )}
        </div>
        ...
        <div className="col-span-1 md:col-span-2 lg:col-span-4">
          ...
          />
          {errors.objeto && (
            <p className="text-red-500 text-sm">{errors.objeto.message}</p>
          )}
        </div>
        ...
        <div className="md:col-span-1 lg:col-span-2">
          ...
          {errors.contratado_id && (
            <p className="text-red-500 text-sm">
              {errors.contratado_id.message}
            </p>
          )}
        </div>
        ...
        <div className="md:col-span-1 lg:col-span-2">
          ...
          {errors.gestor_id && (
            <p className="text-red-500 text-sm">{errors.gestor_id.message}</p>
          )}
        </div>
        ...
        <div className="md:col-span-1 lg:col-span-2">
          ...
          {errors.fiscal_id && (
            <p className="text-red-500 text-sm">{errors.fiscal_id.message}</p>
          )}
        </div>
          ...
          <div className="mt-1">
            <SearchableSelect
              ...
              onValueChange={(value) => {
                ...
                const nome =
                  termosContratuaisCatalog.find((t) => String(t.id) === value)
                    ?.nome ?? "";
                ...
                <div className="flex justify-between items-center mb-3">
                  ...
                  <div className="flex gap-2">
                    <span className="text-xs text-gray-500">
                      Total:{" "}
                      {(getTotalFileSize(selectedFiles) / 1024 / 1024).toFixed(
                        2,
                      )}{" "}
                      MB / 50 MB
                    </span>
                  ...
                  <div className="mt-2 text-xs">
                    <div className="w-full bg-gray-200 rounded-full h-1.5">
                      <div
                        ...
                        style={{
                          width: `${Math.min((getTotalFileSize(selectedFiles) / MAX_TOTAL_SIZE) * 100, 100)}%`,
                        }}
                      ></div>
                    </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewUserData((prev) => ({ ...prev, cpf: e.target.value }))
                  }
                  className="w-full px-3 py-2 border border-blue-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Pode usar com ou sem formatação (pontos e hífen)
                </p>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Perfis * (selecione um ou mais)
                </label>
                <div className="space-y-2 max-h-32 overflow-y-auto border border-blue-200 rounded-lg p-3 bg-blue-50/30">
                  {perfis.map((perfil) => (
                    <label
                      key={perfil.id}
                      className="flex items-center space-x-2 cursor-pointer hover:bg-blue-100/50 p-1 rounded"
                    >
                      <input
                        type="checkbox"
